...
def target(x): # evolution's target
    return x*x*x*x + x*x*x + x*x + x + 1

def generate_dataset(): # generate 101 data points from target
    dataset = []
    for x in range(-100,101,2): 
        x /= 100
        dataset.append([x, target(x)])
    return dataset
...
class GPTree:
    ...
    def print_tree(self, prefix = ""): # textual printout
        ...
        if self.left:  self.left.print_tree ("left: "+ prefix + "   ")
